...
# old-man narrator). 16 = 九州そら/ノーマル: mature & clear. Speed 1.5 keeps a brisk,
# lively pace (VOICEVOX speedScale preserves pitch, so it stays graceful, just faster).
INTRO_VOICE = (16, 1.5)
...
def load_intro():
    ...
    if not m:
        return []
    return re.findall(r'jp\s*:\s*"((?:[^"\\]|\\.)*)"', m.group(1))
