...
python3 tools/gen_audio.py                  # 生成 audio/*.mp3 + audio/manifest.json
python3 tools/gen_audio.py --list-speakers  # 换声音
```

脚本**幂等**（已存在则跳过）。生成后用本地服务器打开，网页自动优先播放 MP3、缺文件回退 TTS；速度滑块(0.5–1.1×)对真人音频同样有效。**名句想用真人原声**：把同名 MP3（如 `audio/d17_s7.mp3`）放进 `audio/` 并加到 `manifest.json` 即可覆盖。
